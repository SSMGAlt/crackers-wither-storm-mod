...
public class StormSpawnPlatformStructure extends Structure {
   ...
   public Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
      if (isFeatureChunk(context, this)) {
         ...
         return Optional.of(
            new GenerationStub(
               pos, builder -> builder.addPiece(new StormSpawnPlatformStructure.Piece(context.structureTemplateManager(), structure, pos, rotation, 0))
            )
         );
      } else {
   ...
   public static class Piece extends TemplateStructurePiece {
      ...
      public Piece(StructureTemplateManager manager, CompoundTag nbt) {
         super(WitherStormModStructures.PLATFORM, nbt, manager, id -> {
            ...
            return makeSettings(Rotation.valueOf(nbt.getString("Rot")), new BlockPos(template.getSize().getX() / 2, 0, template.getSize().getZ() / 2));
         });
