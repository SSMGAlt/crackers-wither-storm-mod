...
public class AnvilRecipeBuilder implements RecipeBuilder {
   ...
   public static AnvilRecipeBuilder commandBlockTool(ItemLike requirement, ItemLike result) {
      return new AnvilRecipeBuilder(
         Ingredient.of(new ItemLike[]{requirement}), Ingredient.of(new ItemLike[]{(ItemLike)WitherStormModItems.COMMAND_BLOCK_BOOK.get()}), result, 1
      );
   }
   ...
   public record Result(ResourceLocation id, String group, Ingredient left, Ingredient right, Item result, int xpCost) implements FinishedRecipe {
